...
test.describe('Provider Management Happy Path', () => {
  ...
  test('should complete full provider lifecycle successfully', async () => {
    ...
    // Wait for modal to close instead of URL change since we're already on providers page
    await expect(providersPage.pageInstance.locator('[role="dialog"]')).not.toBeVisible();
    await providersPage.pageInstance.waitForTimeout(2000); // Give time for provider to appear in list
