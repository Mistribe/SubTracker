...
type TokenGenerator interface {
	...
	GetClient() *openapi2.APIClient
}

func MakeRequest[TOut any](
	generator TokenGenerator,
	call func(ctx context.Context, client *openapi2.APIClient) (TOut, *http.Response, error)) (TOut, error) {
	...
	ctx = context.WithValue(ctx, openapi2.ContextAccessToken, token)
...
func (k *kindeService) GetClient() *openapi2.APIClient {
	// Create configuration
	cfg := openapi2.NewConfiguration()

	// Set server URL (customize the subdomain)
	cfg.Servers = openapi2.ServerConfigurations{
		{
			URL:         k.domain,
	...
	// Create API client
	client := openapi2.NewAPIClient(cfg)
