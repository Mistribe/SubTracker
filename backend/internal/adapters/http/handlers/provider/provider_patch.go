...
type ProviderPatchEndpoint struct {
	handler ports.CommandHandler[command.PatchProviderCommand, provider.Provider]
}

func NewProviderPatchEndpoint(handler ports.CommandHandler[command.PatchProviderCommand, provider.Provider]) *ProviderPatchEndpoint {
...
func (m patchPriceModel) Price() (provider.Price, error) {
	...
	updatedAt := x.ValueOrDefault(m.UpdatedAt, createdAt)
