...
import (
	...
	cache2 "github.com/mistribe/subtracker/internal/adapters/cache"
	"github.com/mistribe/subtracker/internal/infrastructure/cache"
)

func TestSetAndGet(t *testing.T) {
	cfg := config.NewConfiguration()
	local := cache2.NewLocal(cfg)
	...
	t.Run("Set and Get with expiration", func(t *testing.T) {
		local.Set("key2", "value2", cache.WithDuration(100*time.Millisecond))
	...
	t.Run("Get expired item", func(t *testing.T) {
		local.Set("key3", "value3", cache.WithDuration(1*time.Millisecond))
...
func TestSetAndGet_WithExpiration(t *testing.T) {
	...
	t.Run("Set with custom TTL overriding default", func(t *testing.T) {
		local.Set("key2", "value2", cache.WithDuration(200*time.Millisecond))
		if got := local.Get("key2"); got != "value2" {
			t.Errorf("Get() = %v, want %v", got, "value2")
		}
		time.Sleep(150 * time.Millisecond)
	...
	t.Run("Multiple items with different expirations", func(t *testing.T) {
		local.Set("key3", "value3", cache.WithDuration(300*time.Millisecond))
		local.Set("key4", "value4", cache.WithDuration(50*time.Millisecond))
		time.Sleep(100 * time.Millisecond)
