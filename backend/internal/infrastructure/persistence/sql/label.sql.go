...
const getLabelsForOwner = `-- name: GetLabelsForOwner :many
SELECT id, owner_type, owner_family_id, owner_user_id, name, key, color, created_at, updated_at, etag, COUNT(*) OVER () AS total_count
FROM public.labels l
...
type GetLabelsForOwnerParams struct {
	OwnerType     string
	OwnerUserID   *string
	OwnerType_2   string
	OwnerFamilyID *uuid.UUID
...
type GetLabelsForOwnerRow struct {
	ID            uuid.UUID
	OwnerType     string
	OwnerFamilyID *uuid.UUID
	OwnerUserID   *string
...
func (q *Queries) GetLabelsForOwner(ctx context.Context, arg GetLabelsForOwnerParams) ([]GetLabelsForOwnerRow, error) {
	rows, err := q.db.Query(ctx, getLabelsForOwner,
		arg.OwnerType,
		arg.OwnerUserID,
		arg.OwnerType_2,
		arg.OwnerFamilyID,
	...
	defer rows.Close()
	var items []GetLabelsForOwnerRow
	for rows.Next() {
		var i GetLabelsForOwnerRow
		if err := rows.Scan(
			&i.ID,
			&i.OwnerType,
			&i.OwnerFamilyID,
