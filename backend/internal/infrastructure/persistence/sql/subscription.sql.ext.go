...
func (q *Queries) GetSubscriptions(ctx context.Context, limit, offset int32) ([]SubscriptionRow, int64, error) {
	rows, err := q.getSubscriptions(ctx, getSubscriptionsParams{Limit: limit, Offset: offset})
	if err != nil {
		return nil, 0, err
	}
	if len(rows) == 0 {
